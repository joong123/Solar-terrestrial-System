...
void CreateSphere(ID3DXMesh ** obj, int finess, float radius, D3DCOLOR color)
{
	...
	for (face = 1, point = 1; point <= 2 * loops; face++, point++)//第一环
	{
		...
		g_vertices[point].pos
			= D3DXVECTOR3(radius*cos(hradian)*sin(vradian), radius*sin(hradian)*sin(vradian), -height - radius - radius*cos(vradian));
		g_vertices[point].normal
			= D3DXVECTOR3(cos(hradian)*sin(vradian), sin(hradian)*sin(vradian), -cos(vradian));
	...
	//最后一点
	g_vertices[point].pos = D3DXVECTOR3(0.0, 0.0, -height);
	g_vertices[point].normal = D3DXVECTOR3(0.0, 0.0, 1.0);
